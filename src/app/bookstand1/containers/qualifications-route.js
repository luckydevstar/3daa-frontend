...
import WorkbookOpenCard from '../components/workbooks/workbook-open-card';
...
class QualificationsRoute extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    const { qualifications, activeLevel } = nextProps;
  ...
  renderQualifications() {
    ...
    if (lodash.size(filteredQualifications)) {
      ...
      return (
        <div className="container bookstand-container min-content-height-inner animated fadeIn">
          <div className="unit-workbook-card-view">
            {filteredUnits.map((u, i) => (
              <div
                className="unit-workbook-card"
                key={'unit_workbook_card' + i}
              >
                <WorkbookOpenCard
                  workbook={u.workbook}
                  selectedWorkbooks={[]}
                  ...
                  openWorkbookPreview={() => {
                    browserHistory.push(
                      `bookstand/preview/${u.unit_id}/${u.workbook.workbook_id}`
                    );
                  }}
                />
              </div>
  ...
  renderActiveQualification() {
    ...
    const learnerId = path(['params', 'learnerId'], router);
      ...
      <div className="container bookstand-container min-content-height-inner animated fadeBig">
        <div className="active-qualification-container">
          ...
          <div className="qualification-details">
            ...
            {!learnerId && (
              <UserAccess allowRoles={[SuperAdmin, CentreTutor]}>
                <Link to="/community/learners">
                  Select a learner in the community.
                </Link>
              </UserAccess>
            )}
          </div>
...
function mergeProps(stateProps, dispatchProps, ownProps) {
  ...
  return {
    ...
    setActiveQualification: qualification => {
      dispatch(BookstandCreators.setActiveQualification(qualification));
    },
...
export default connect(mapStateToProps, null, mergeProps)(QualificationsRoute);
