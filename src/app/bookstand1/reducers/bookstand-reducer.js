...
const extractAllWorkbooks = unit => {
  let workbooks = lodash.get(unit, 'workbooks', []);
  workbooks = workbooks.map(wb => {
    wb = lodash.merge(wb, lodash.pick(unit, workbookUnitKeys));
    if (!lodash.get(wb, 'cover')) {
      wb.cover = unit.cover;
    }
    return wb;
  });
...
const getSectorQualificationSuccess = (state, { qualification }) => {
  ...
  return state.merge({
    uiGettingQualification: false,
    qualification: pipe(
      assoc('workbooks', currentWorkbooks),
      omit('units')
    )(qualification)
  });
};
...
const getCentreTutorQualificationSuccess = (state, { qualification }) => {
  ...
  return state.merge({
    uiGettingQualification: false,
    qualification: pipe(
      assoc('workbooks', selectedWorkbooks),
      omit('units')
    )(qualification)
  });
};
...
const setActiveQualification = (state, { qualification }) => {
  return state.merge({
    qualification
  });
};
